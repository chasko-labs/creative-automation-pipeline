...
- Style tokens live in `design/tokens/kodiak.json` and mirror to cloud. The pipeline pulls them first from cloud when `DAM_S3_BUCKET` is set, otherwise from your laptop. No hard-coded colors. Panda CSS consumes them. `npm run tokens:config` regenerates `panda.config.ts` via `scripts/gen-panda-config.py`; `npm run tokens:css` rebuilds the stylesheet. `scripts/check-panda-tokens.py` blocks merges when generated files differ from `design/tokens/kodiak.json`.
