...
- When a photo exists, the system reuses it and composes an on-brand hero — the real product image on the frontier palette with negative space for the line, headline and layout driven by Amazon Nova Pro (`amazon.nova-pro-v1:0`, Converse, us-east-1) reading the actual pack shot. When no real asset is found or credentials are off, it falls back to a deterministic Wasatch-palette placeholder. Missed heroes are never blank. This is deliberate: grounding every hero in your real, rights-clean product photography and composing with Nova Pro vision keeps the output unmistakably on-brand — the frontier palette, the bear, the exact pack shot — rather than approximating the brand with generated pixels.
