...
- When a photo exists, the system reuses it and composes an on-brand hero — the real product image on the frontier palette with negative space for the line, headline and layout driven by Amazon Nova Pro (`amazon.nova-pro-v1:0`, Converse, us-east-1) reading the actual pack shot. When no real asset is found or credentials are off, it falls back to a deterministic Wasatch-palette placeholder. Missed heroes are never blank. Note: Amazon has no live text-to-image generator here — Nova Canvas is retired (provider-marked Legacy) and Stability generators are org-SCP denied — so heroes are built from real assets, not generated from scratch.
