...
- Style tokens live in `design/tokens/kodiak.json` and mirror to cloud. The pipeline pulls them first from cloud when `ASSET_STORE_S3_BUCKET` is set, otherwise from your laptop. No hard-coded colors. Panda CSS consumes them. `npm run tokens:config` regenerates `panda.config.ts` via `scripts/gen-panda-config.py`; `npm run tokens:css` rebuilds the stylesheet. `scripts/check-panda-tokens.py` blocks merges when generated files differ from `design/tokens/kodiak.json`.
- The image engine runs three modes, and it always names which one produced a given hero. **Mode 1 — the live primary — is a genuine generative restyle seeded by a real photo.** A rights-clean Kodiak asset store lifestyle shot goes in as the seed; Amazon Nova Pro (vision, Converse, `amazon.nova-pro-v1:0`, us-east-1) reads that photo and art-directs a scene prompt for the theme; then Amazon Bedrock Stability control-structure (`us.stability.stable-image-control-structure-v1:0`, control strength 0.7) restyles the real photo so the theme lands in the pixels while the original composition is preserved. The brand's real assets go in, on-theme GenAI comes out — not text-to-image from scratch, not a flat composite. Source label: `bedrock:stability-control-structure`. On top of that render a deterministic Pillow brand overlay is composited — headline plus the Blaze Orange accent bar — and a roughly 2% kraft-paper texture is baked in so the mark and the grain are exact every time.
...
Runbooks: [AgentCore](docs/agentcore.md) · [Asset library runbook](docs/asset-library-runbook.md) · [Observability runbook](docs/observability-runbook.md) · [Linda Film Crew pattern](docs/linda-film-crew-pattern.md)
