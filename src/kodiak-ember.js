...
// undefined and every shared-engine mount throws at registerSceneView.
...
// ── Wave 3 — frontier marquee sign ──
// A Fallout-New-Vegas-on-cardboard marquee for the index header: a kraft board
// face (DynamicTexture, stenciled headline) ringed by chasing LED bulbs
// (emissive spheres + GlowLayer), washed by a RectAreaLight in the manner of
// the area-lights playground study, with four live status plates beneath —
// MARKET / SEASON / PREVIEW / PACK — that mirror the page's own labels and
// timeline states via MutationObserver. A warm point-light wash stands in for
// the playground's area-light study (RectAreaLight cannot be aimed: Babylon
// lights are Nodes, not TransformNodes). Light/dark follows
// prefers-color-scheme (face + ink redraw, bulb intensity shifts); the static
// HTML headline plate is the default and stands on gate-fail or any error.
// Runs ONLY on the shared engine singleton; reduced-motion holds the chase
// static (alternating lit bulbs, no animation).

/** Marquee geometry, shared by the sign class + bulb layout. @type {object} */
const SIGN_GEOM = {
	boardW: 11,
	boardH: 3.4,
	boardY: 1.2,
	...
	plateH: 1.05,
	plateY: -1.9,
	plateXs: [-4.06, -1.35, 1.35, 4.06],
};

/**
 * Draw the main board face: cardboard ground, stenciled headline, rivet dots.
 * @param {CanvasRenderingContext2D} ctx canvas 2d context of the face texture
 * @param {number} w texture width in px
 * @param {number} h texture height in px
 * @param {boolean} dark true for the dark-scheme face
 */
function signDrawFace(ctx, w, h, dark) {
	const bg = dark ? "#3B2316" : "#D9BE97";
	const ink = dark ? "#FFF8F0" : "#3B2316";
	const accent = "#E8530E";
	ctx.fillStyle = bg;
	ctx.fillRect(0, 0, w, h);
	// kraft fiber streaks — horizontal hairlines, never lines that read as rules
	ctx.strokeStyle = dark ? "rgba(255,248,240,0.05)" : "rgba(59,35,22,0.07)";
	ctx.lineWidth = 1;
	for (let y = 8; y < h; y += 9) {
		ctx.beginPath();
		ctx.moveTo(0, y);
	...
	ctx.textAlign = "center";
	ctx.fillStyle = ink;
	ctx.font = `800 ${Math.round(h * 0.3)}px Georgia, "Times New Roman", serif`;
	ctx.fillText("KODIAK", w / 2, h * 0.44);
	ctx.fillStyle = accent;
	ctx.font = `700 ${Math.round(h * 0.13)}px Georgia, serif`;
	ctx.fillText("POSTS FOR TODAY'S FRONTIER", w / 2, h * 0.66);
	ctx.fillStyle = ink;
	ctx.font = `${Math.round(h * 0.075)}px Georgia, serif`;
	ctx.fillText("KEEP IT WILD", w / 2, h * 0.82);
}

/**
 * Draw one status plate: label rail + live value.
 * @param {CanvasRenderingContext2D} ctx canvas 2d context of the plate texture
 * @param {number} w texture width in px
 * @param {number} h texture height in px
 * @param {string} label plate label (MARKET / SEASON / PREVIEW / PACK)
 * @param {string} value live value line
 * @param {boolean} warn true renders the value in blaze (locked/warning state)
 * @param {boolean} dark true for the dark-scheme face
 */
function signDrawPlate(ctx, w, h, label, value, warn, dark) {
	const bg = dark ? "#2A190F" : "#3B2316";
	const ink = dark ? "#FFF8F0" : "#FFEDE0";
	ctx.fillStyle = bg;
	ctx.fillRect(0, 0, w, h);
	ctx.strokeStyle = warn ? "#E8530E" : "#FF8A3D";
	ctx.lineWidth = 4;
	...
	ctx.fillStyle = warn ? "#E8530E" : ink;
	ctx.font = `${Math.round(h * 0.21)}px Georgia, serif`;
	const clipped = value.length > 22 ? value.slice(0, 21) + "…" : value;
	ctx.fillText(clipped, w / 2, h * 0.68);
}

class FrontierSign {
	/**
	 * @param {HTMLCanvasElement} canvas overlay canvas (absolute, transparent)
	 * @param {HTMLElement} host the #frontierSign header host
	 */
	constructor(canvas, host) {
		this.canvas = canvas;
		this.host = host;
		this.engine = ensureEngine();
		this._reduceMotion = prefersReducedMotion();
		this._dark = matchMedia("(prefers-color-scheme: dark)").matches;
		this.scene = new Scene(this.engine);
		this.scene.clearColor = new Color4(0, 0, 0, 0); // transparent — static plate behind
		this.camera = new FreeCamera("signCam", new Vector3(0, 0.2, -11.5), this.scene);
		this.camera.setTarget(new Vector3(0, 0, 0));
		// base fill so the kraft body reads on first frame, before the wash lands
		this.hemi = new HemisphericLight("signHemi", new Vector3(0, 1, -1), this.scene);
		this.hemi.intensity = 0.5;
		this.hemi.diffuse = PARCHMENT;
		this.hemi.groundColor = hex("#5A4232");
		// the Vegas wash: a warm point light above-front rakes the kraft face and
		// bulb ring. (RectAreaLight was tried here — Babylon lights are Nodes,
		// not TransformNodes, so there is no rotation with which to aim its
		// fixed -Z emission at the board; a PointLight needs no aiming.)
		this.wash = new PointLight("signWash", new Vector3(0, 2.5, -4), this.scene);
		this.wash.diffuse = AMBER;
		this.wash.specular = PARCHMENT;
		this.wash.intensity = 0.9;
		this.wash.range = 30;
		// cardboard body + printed face
		const body = MeshBuilder.CreateBox(
			"signBody",
			{ width: SIGN_GEOM.boardW + 0.3, height: SIGN_GEOM.boardH + 0.3, depth: SIGN_GEOM.bodyDepth },
			this.scene,
		);
		body.position = new Vector3(0, SIGN_GEOM.boardY, 0);
		const bodyMat = new StandardMaterial("signBodyMat", this.scene);
		bodyMat.diffuseColor = BEAR;
		bodyMat.specularColor = new Color3(0.04, 0.04, 0.04);
		body.material = bodyMat;
		this.faceTex = new DynamicTexture("signFace", { width: 1024, height: 320 }, this.scene, false);
		const face = MeshBuilder.CreatePlane(
			"signFace",
			{ width: SIGN_GEOM.boardW, height: SIGN_GEOM.boardH },
			this.scene,
		);
		face.position = new Vector3(0, SIGN_GEOM.boardY, SIGN_GEOM.faceZ);
		// NO rotation: CreatePlane already faces the -Z camera. A half-turn
		// aims the textured side away and the board renders as a dark void
		// (verified on hardware: rotation.y = Math.PI hides the face).
		const faceMat = new StandardMaterial("signFaceMat", this.scene);
		faceMat.diffuseTexture = this.faceTex;
		faceMat.specularColor = new Color3(0.03, 0.03, 0.03);
		face.material = faceMat;
		this.redrawFace();
		// LED bulb ring — two shared materials, chase swaps them in thirds
		this.litMat = new StandardMaterial("signBulbLit", this.scene);
		this.litMat.diffuseColor = new Color3(0.25, 0.12, 0.04);
		this.litMat.emissiveColor = AMBER;
		this.dimMat = new StandardMaterial("signBulbDim", this.scene);
		this.dimMat.diffuseColor = new Color3(0.23, 0.14, 0.09);
		this.dimMat.emissiveColor = new Color3(0.05, 0.03, 0.02);
		this.bulbs = this.buildBulbs();
		this.glow = new GlowLayer("signGlow", this.scene, { blurKernelSize: 32 });
		this.glow.intensity = this._dark ? 1.3 : 0.9;
		// status plates — MARKET / SEASON / PREVIEW / PACK, live from the page
		this.plates = this.buildPlates();
		this.refreshPlates();
		this.attachSchemeListener();
	...
	// Fit the fixed camera to the host aspect: the full group (bulbs top to
	// plates bottom) always fills the cabinet height, centered horizontally —
	// readable at 375px mobile and ultrawide alike. Re-run on window resize.
	fitCamera() {
		const w = this.canvas.clientWidth || 1;
		const h = this.canvas.clientHeight || 1;
		const aspect = w / h;
		const halfFovTan = Math.tan(0.4); // FreeCamera default fov 0.8
		const needH = 6.4; // bulbs top (3.15) to plates bottom (-2.45) + margin
		const needW = 12.8; // board + bulb ring + margin
		const d = Math.max(needH / (2 * halfFovTan), needW / (2 * halfFovTan * aspect));
		this.camera.position = new Vector3(0, 0.2, -d);
	}

	attachResize() {
		this._onResize = () => this.fitCamera();
		window.addEventListener("resize", this._onResize);
	}

	/** @returns {import("@babylonjs/core/Meshes/mesh").Mesh[]} the bulb meshes in ring order */
	buildBulbs() {
		const bulbs = [];
		const x0 = -(SIGN_GEOM.boardW / 2) - 0.25;
		const x1 = SIGN_GEOM.boardW / 2 + 0.25;
		const yTop = SIGN_GEOM.boardY + SIGN_GEOM.boardH / 2 + 0.25;
		const yBot = SIGN_GEOM.boardY - SIGN_GEOM.boardH / 2 - 0.25;
		const step = 0.66;
		const spots = [];
		...
		for (let y = yBot + step; y < yTop - 0.01; y += step) {
			spots.push([x0, y]);
			spots.push([x1, y]);
		}
		spots.forEach(([x, y], i) => {
			const bulb = MeshBuilder.CreateSphere(`signBulb${i}`, { diameter: SIGN_GEOM.bulbDia, segments: 8 }, this.scene);
			bulb.position = new Vector3(x, y, SIGN_GEOM.bulbZ);
			bulb.material = i % 3 === 0 ? this.litMat : this.dimMat;
			bulbs.push(bulb);
		});
		return bulbs;
	}

	/** @returns {{label: string, tex: DynamicTexture}[]} plates left to right */
	buildPlates() {
		const defs = ["MARKET", "SEASON", "PREVIEW", "PACK"];
		return SIGN_GEOM.plateXs.map((x, i) => {
			const tex = new DynamicTexture(`signPlate${i}`, { width: 256, height: 112 }, this.scene, false);
			const plate = MeshBuilder.CreatePlane(
				`signPlateMesh${i}`,
				{ width: SIGN_GEOM.plateW, height: SIGN_GEOM.plateH },
				this.scene,
			);
			plate.position = new Vector3(x, SIGN_GEOM.plateY, SIGN_GEOM.faceZ);
			// same as the main face: planes already face the -Z camera.
			const mat = new StandardMaterial(`signPlateMat${i}`, this.scene);
			mat.diffuseTexture = tex;
			mat.specularColor = new Color3(0.03, 0.03, 0.03);
			plate.material = mat;
			return { label: defs[i], tex };
		});
	}

	redrawFace() {
		const ctx = this.faceTex.getContext();
		signDrawFace(ctx, 1024, 320, this._dark);
		this.faceTex.update();
	}

	/** Read the page's own labels + timeline states into the four plates. */
	refreshPlates() {
		/** @param {string} id element id @param {string} fallback */
		const text = (id, fallback) => {
			const el = document.getElementById(id);
			const t = el ? String(el.textContent || "").trim() : "";
			return t || fallback;
		};
		/** @param {string} node timeline data-node @param {string} fallback */
		const nodeState = (node, fallback) => {
			const li = document.querySelector(`#progressTimeline li[data-node="${node}"]`);
			const s = li ? String(li.getAttribute("data-state") || "").toUpperCase() : "";
			return s || fallback;
		};
		const values = [
			text("locationSectionLabel", "Park City, Utah"),
			text("seasonSectionLabel", "September"),
			nodeState("preview", "QUEUED"),
			nodeState("assets", "QUEUED"),
		];
		this.plates.forEach((plate, i) => {
			const warn = plate.label !== "MARKET" && plate.label !== "SEASON" && values[i] === "LOCKED";
			const ctx = plate.tex.getContext();
			signDrawPlate(ctx, 256, 112, plate.label, values[i], warn, this._dark);
			plate.tex.update();
		});
	}
	...
	attachSchemeListener() {
		...
		if (this._schemeQuery.addEventListener) this._schemeQuery.addEventListener("change", this._onScheme);
	}

	attachPageObservers() {
		if (typeof MutationObserver === "undefined") return;
		this._pageObserver = new MutationObserver(() => this.refreshPlates());
		const labelCfg = { childList: true, characterData: true, subtree: true };
		for (const id of ["locationSectionLabel", "seasonSectionLabel"]) {
			const el = document.getElementById(id);
			if (el) this._pageObserver.observe(el, labelCfg);
		}
		const timeline = document.getElementById("progressTimeline");
		if (timeline) this._pageObserver.observe(timeline, { attributes: true, subtree: true, attributeFilter: ["data-state"] });
	}

	attachChase() {
		if (this._reduceMotion) return; // static alternating bulbs, no animation
		let acc = 0;
		let phase = 0;
		this.scene.registerBeforeRender(() => {
			acc += this.scene.getEngine().getDeltaTime();
			if (acc < 140) return; // ~7 chase steps per second — marquee tempo, not strobe
			acc = 0;
			phase = (phase + 1) % 3;
			for (let i = 0; i < this.bulbs.length; i++) {
				this.bulbs[i].material = (i + phase) % 3 === 0 ? this.litMat : this.dimMat;
			}
		});
	}
	...
	dispose() {
		...
		if (this._schemeQuery && this._onScheme && this._schemeQuery.removeEventListener) {
			this._schemeQuery.removeEventListener("change", this._onScheme);
		}
...
// mountFrontierSign — lazy, gated mount of the marquee over the index header
// plate. The static HTML headline plate is the DEFAULT visual: the host only
// gains .is-live (which hides the static plate) after the 3D scene constructs
// successfully, so gate-fail or any error leaves the headline standing.
function mountFrontierSign(selector = "#frontierSign") {
	const host =
		typeof selector === "string"
			? document.querySelector(selector)
			: selector;
	...
	const state = { sign: null, mounted: false };

	const tryMount = () => {
		if (state.mounted) return; // never leak a second scene
		const reason = gateReason(host);
		if (reason) {
			console.info("KodiakEmber frontier-sign:", reason.message);
			return;
		}
		const canvas = document.createElement("canvas");
		canvas.className = "frontier-sign__canvas";
		canvas.style.cssText =
		...
		host.appendChild(canvas);
		try {
			state.sign = new FrontierSign(canvas, host);
			state.mounted = true;
			host.classList.add("is-live"); // 3D up — retire the static plate
		} catch (err) {
			// construction failure — one diagnostic, then remove the dead
			// canvas and leave the static plate standing.
			console.info("KodiakEmber frontier-sign: 3D construction failed —", err && err.message ? err.message : err);
			state.mounted = false;
			canvas.remove();
		}
	};
	...
	if ("IntersectionObserver" in window) {
		const io = new IntersectionObserver(
			...
			{ rootMargin: "200px 0px" }, // header is at top — mounts on first paint
		);
		io.observe(host);
	} else {
		tryMount();
