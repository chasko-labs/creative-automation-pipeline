...
class AssetLibrary:
    ...
    def _require_s3(self) -> Any:
        if self._s3 is None:
            raise RuntimeError(
                "AssetLibrary is in no-s3 mode: boto3/creds unavailable, writes disabled"
            )
        return self._s3
    ...
    def add_asset(
        ...
        *,
        ...
        try:
            ...
            self.obs.log_event(
                "asset.reject", level="warn", filename=filename, reason="unsupported_kind"
            )
            raise
        ...
        asset_id = (
            uuid.uuid4().hex
        )  # uuid4 hex: no ulid dep pulled in; time-sort deferred to DynamoDB
