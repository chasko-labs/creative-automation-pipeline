"""asset store lookup — S3-first with local input_assets fallback.

Priority:
1. explicit path (local or s3://)
2. S3 asset store (if ASSET_STORE_S3_BUCKET / ASSET_STORE_S3_URI env set and boto3/creds available) — fetch to local cache
3. local asset_root fallback (input_assets/<product_id>/...)

Runbook sync (see docs/asset-library-runbook.md):
  # pull S3 -> local (preferred for offline / CI)
  aws s3 sync s3://$ASSET_STORE_S3_BUCKET/dam input_assets --delete --only-show-errors
  # push local -> S3 (publish new assets)
  aws s3 sync input_assets s3://$ASSET_STORE_S3_BUCKET/dam --delete --only-show-errors
"""
...
def _env_compat(new: str, old: str, default: str = "") -> str:
    """Read the asset-store env name, falling back to the legacy name.

    Already-deployed environments still export the legacy names; new
    deployments should export the new ones. New wins when both are set.
    """
    return os.getenv(new, "").strip() or os.getenv(old, "").strip() or default
...
ASSET_STORE_PROBE_FLOOR_MS = int(_env_compat("ASSET_STORE_PROBE_FLOOR_MS", "DAM_PROBE_FLOOR_MS", "3000"))
...
def _deadline_exceeded(deadline_ms) -> bool:
    ...
    if deadline_ms is None:
        return False
    try:
        return deadline_ms() < ASSET_STORE_PROBE_FLOOR_MS
...
# override (ASSET_STORE_CACHE_ROOT) wins for callers that need a bespoke cache location.
_DEFAULT_CACHE_ROOT = "/tmp/kodiak-assets"


def _asset_store_cache_root() -> Path:
    """Writable root for S3-fetch downloads: $ASSET_STORE_CACHE_ROOT, else /tmp/kodiak-assets.

    Never returns the read-only relative input_assets. Local reads still hit the
    caller-supplied asset_root; only the fetched-asset WRITE destination moves here.
    """
    return Path(_env_compat("ASSET_STORE_CACHE_ROOT", "DAM_CACHE_ROOT", "") or _DEFAULT_CACHE_ROOT)


def _s3_bucket_and_prefix() -> tuple[str | None, str]:
    """Resolve bucket/prefix from ASSET_STORE_S3_BUCKET + ASSET_STORE_S3_PREFIX or ASSET_STORE_S3_URI.

    Examples:
      ASSET_STORE_S3_BUCKET=my-bucket ASSET_STORE_S3_PREFIX=dam/  -> (my-bucket, dam/)
      ASSET_STORE_S3_URI=s3://my-bucket/dam               -> (my-bucket, dam/)
    """
    uri = _env_compat("ASSET_STORE_S3_URI", "ASSET_STORE_S3_URI", "")
    ...
    bucket = _env_compat("ASSET_STORE_S3_BUCKET", "ASSET_STORE_S3_BUCKET", "") or None
    prefix = _env_compat("ASSET_STORE_S3_PREFIX", "ASSET_STORE_S3_PREFIX", "dam/")
...
def _s3_client():
    if not _s3_enabled():
        return None
    region = _env_compat("ASSET_STORE_S3_REGION", "DAM_S3_REGION", os.getenv("BEDROCK_REGION", os.getenv("AWS_REGION", "us-east-1")))
    try:
        ...
        read_timeout = int(_env_compat("ASSET_STORE_S3_READ_TIMEOUT_S", "DAM_S3_READ_TIMEOUT_S", "2"))
        connect_timeout = int(_env_compat("ASSET_STORE_S3_CONNECT_TIMEOUT_S", "DAM_S3_CONNECT_TIMEOUT_S", "1"))
        cfg = None
...
def _s3_download(bucket: str, key: str, dest: Path) -> bool:
    ...
    try:
        try:
            ...
            # candidate / local path. The writable _asset_store_cache_root default should keep
            # this from firing on Lambda, but the guard is the belt to that suspenders.
            print(f"[asset-store] s3 cache dir unwritable {dest.parent}: {e}")
            return False
        ...
        print(f"[asset-store] s3 fetch miss s3://{bucket}/{key}: {e}")
        return False


def _s3_try_fetch_product_asset(product_id: str, asset_root: Path, explicit: str | None = None, deadline_ms=None) -> Path | None:
    ...
    s3://<bucket>/<prefix><product_id>/ . A pre-synced hit under asset_root/<product_id>/
    is honored first (local read), but fetched downloads are written to
    _asset_store_cache_root()/<product_id>/ — never the (possibly read-only) asset_root — so the
    ...
    HERO_NAMES fan-out: the loop ABANDONS once remaining < ASSET_STORE_PROBE_FLOOR_MS.
    """
    if not _s3_enabled():
        return None
    cache_root = _asset_store_cache_root()
    ...
    if explicit and explicit.startswith("s3://"):
        ...
        if not key:
            return None
        fname = Path(key).name
        # honor a pre-synced local copy under asset_root, else download to the cache root
        local_pre = asset_root / product_id / fname
        if local_pre.exists():
            return local_pre
        ...
        if _s3_download(bucket, key, dest):  # type: ignore
            print(f"[asset-store] s3 hit {explicit} -> {dest}")
            return dest
        return None
    ...
    for ext in ASSET_EXTS:
        for name in HERO_NAMES:
            if _deadline_exceeded(deadline_ms):
                print(f"[asset-store] product probe abandoned mid-fan-out (budget floor) s3://{bucket}/{base_prefix}")
                return None
            key = f"{base_prefix}{name}{ext}"
            # pre-synced local copy under asset_root wins (offline / CI), else cache root
            local_pre = asset_root / product_id / f"{name}{ext}"
            if local_pre.exists():
                return local_pre
            ...
            if _s3_download(bucket, key, dest):
                print(f"[asset-store] s3 hit s3://{bucket}/{key}")
                return dest
            ...
            if dest.exists() and dest.stat().st_size == 0:
                try:
                    ...
                    print(f"[asset-store] partial cleanup unlink failed {dest}: {e}")

    # 2) any image under product prefix — list
    if _deadline_exceeded(deadline_ms):
        print(f"[asset-store] product list probe skipped (budget floor) s3://{bucket}/{base_prefix}")
        return None
    try:
        ...
        for obj in resp.get("Contents", []):
            ...
            if any(key.lower().endswith(ext) for ext in ASSET_EXTS):
                ...
                if dest.exists():
                    return dest
                if _s3_download(bucket, key, dest):
                    print(f"[asset-store] s3 hit (list) s3://{bucket}/{key}")
                    return dest
    except (ClientError, BotoCoreError, Exception) as e:  # noqa: BLE001 — any list miss falls to serial probes
        print(f"[asset-store] s3 list miss s3://{bucket}/{base_prefix}: {e}")
    return None


# Heroes live at a fixed brand path in the asset store, independent of the generic
# ASSET_STORE_S3_PREFIX used for the dam/ style library. api.py, generate_lambda.py and
# the web mcp all cite s3://<bucket>/brands/kodiak/heroes/<product>/hero.png .
# Overridable for other brands / test buckets via ASSET_STORE_HEROES_PREFIX.
def _heroes_prefix() -> str:
    prefix = _env_compat("ASSET_STORE_HEROES_PREFIX", "DAM_HEROES_PREFIX", "brands/kodiak/heroes/")
    if prefix and not prefix.endswith("/"):
        prefix += "/"
    return prefix
...
def fetch_hero_to_tmp(product_id: str, cache_root: Path = Path("/tmp/kodiak-assets"), deadline_ms=None) -> Path | None:
    """Materialize a product hero from the S3 asset store into a Lambda-safe /tmp cache.

    Resolves s3://<asset store bucket>/<heroes prefix><product_id>/hero-real.png first
    ...
    Offline-safe: returns None when S3 is disabled (ASSET_STORE_S3_BUCKET unset or boto3
    ...
    the probe ABANDONS once remaining < ASSET_STORE_PROBE_FLOOR_MS, returning None so the
    ...
    for fname in _HERO_FILE_RANK:
        ...
        try:
            ...
            pass
    if _deadline_exceeded(deadline_ms):
        print(f"[asset-store] hero probe abandoned (budget floor) s3://{bucket}/{base_prefix}")
        return None
    ...
    try:
        ...
        if client is not None:
            ...
            for key in keys:
                if _deadline_exceeded(deadline_ms):
                    print(f"[asset-store] hero probe abandoned mid-list (budget floor) s3://{bucket}/{base_prefix}")
                    return None
                ...
                try:
                    ...
                    pass
                if _s3_download(bucket, key, dest):
                    print(f"[asset-store] hero s3 hit (list) s3://{bucket}/{key} -> {dest}")
                    return dest
                if dest.exists() and dest.stat().st_size == 0:
                    try:
                        dest.unlink()
            ...
            print(f"[asset-store] hero list miss s3://{bucket}/{base_prefix}")
            return None
    except Exception as e:  # noqa: BLE001 — narrow IAM (no ListBucket) etc.
        print(f"[asset-store] hero list unavailable, serial fallback s3://{bucket}/{base_prefix}: {e}")
    # serial fallback — hero-real preferred over hero, deterministic ext order.
    for fname in _HERO_FILE_RANK:
        if _deadline_exceeded(deadline_ms):
            print(f"[asset-store] hero probe abandoned mid-fan-out (budget floor) s3://{bucket}/{base_prefix}")
            return None
        ...
        try:
            ...
            pass
        if _s3_download(bucket, key, dest):
            print(f"[asset-store] hero s3 hit s3://{bucket}/{key} -> {dest}")
            return dest
...
def fetch_asset_key(key: str, dest: Path) -> Path | None:
    """Download an EXPLICIT full asset key to dest. Returns dest if fetched, else None.

    Unlike find_hero_asset / fetch_hero_to_tmp, the key is used VERBATIM against
    ASSET_STORE_S3_BUCKET with NO prefix join — the sku-photo-map ships full keys under
    brands/kodiak/raw-ingest/... which are not under the dam/ ASSET_STORE_S3_PREFIX. The
    ...
    if dest.exists() and dest.stat().st_size > 0:
        return dest
    if _s3_download(bucket, key, dest):
        print(f"[asset-store] key s3 hit s3://{bucket}/{key} -> {dest}")
...
def _load_packshot_map() -> dict:
    ...
    try:
        ...
        print(f"[asset-store] sku-packshot-map load skipped: {e}")
...
def resolve_packshot(product_id: str, asset_root: Path | None = None, deadline_ms=None) -> Path | None:
    ...
    Manifest-first (unlike find_hero_asset). Resolution chain, in order:
      1. manifest packshot_key      -> fetch_asset_key verbatim (no prefix join)
      2. manifest fallbacks[] boxes -> fetch_asset_key per 705599* candidate
      3. fuzzy find_hero_asset      -> S3 prefix + local glob
      4. none                       -> return None (caller falls to generated-scene)

    Offline-safe: fetch_asset_key returns None when S3 is disabled, so local dev / CI
    ...
    if isinstance(entry, dict):
        ...
        for cand in (entry.get("fallbacks") or []) + (entry.get("lifestyle_keys") or []):
            if isinstance(cand, str) and cand.strip() and _looks_like_box(cand):
                ...
                hit = fetch_asset_key(cand, dest)
                if hit is not None:
                    return hit
    # 3) fuzzy glob — reuse the existing S3-prefix + local-glob primitive
    root = asset_root or Path("input_assets")
...
def _s3_try_fetch_brand_logo(asset_root: Path) -> Path | None:
    if not _s3_enabled():
        return None
    bucket, prefix = _s3_bucket_and_prefix()
    if not bucket:
    ...
    for sub in ("brand/", ""):
        ...
        for ext in ASSET_EXTS:
            for name in BRAND_NAMES:
                key = f"{base_prefix}{name}{ext}"
                # cache under brand/ or root; check pre-synced asset_root first, write to cache_root
                rel = f"brand/{name}{ext}" if sub == "brand/" else f"{name}{ext}"
                local_pre = asset_root / rel
                if local_pre.exists():
                    return local_pre
                dest = cache_root / rel
                if dest.exists():
        ...
        try:
            ...
            for obj in resp.get("Contents", []):
                ...
                if any(key.lower().endswith(ext) for ext in ASSET_EXTS):
                    ...
                    local_pre = asset_root / rel_dir / fname if rel_dir else asset_root / fname
                    if local_pre.exists():
                        return local_pre
...
# ------------------------------------------------------------------ local helpers
def _local_find_hero(product_id: str, asset_root: Path, explicit: str | None = None) -> Path | None:
    if explicit:
        p = Path(explicit)
        if p.is_absolute():
            if p.exists():
            ...
            # explicit may be relative to asset_root
            q = asset_root / p
            if q.exists():
                return q
            ...
            if p.exists():
                return p
    product_dir = asset_root / product_id
...
def _local_find_brand(asset_root: Path) -> Path | None:
    for base in [asset_root / "brand", asset_root]:
        if base.is_dir():
            for ext in ASSET_EXTS:
                for name in BRAND_NAMES:
                    c = base / f"{name}{ext}"
...
def find_hero_asset(product_id: str, asset_root: Path, explicit: str | None = None, deadline_ms=None) -> Path | None:
    """Return hero image path if found, else None.

    Lookup order:
    1. explicit path if provided (local or s3:// — s3 fetched to cache)
    2. S3 asset store: s3://$ASSET_STORE_S3_BUCKET/$ASSET_STORE_S3_PREFIX<product_id>/hero.* or any image (cached to asset_root)
    3. local asset_root / product_id / hero.* (any ext)
    4. local asset_root / product_id / any image

    Env:
      ASSET_STORE_S3_BUCKET, ASSET_STORE_S3_PREFIX (default dam/), ASSET_STORE_S3_URI (alt s3://bucket/prefix),
      ASSET_STORE_S3_REGION / BEDROCK_REGION / AWS_REGION, standard AWS credential chain.
    ...
    fan-out so the probe abandons mid-loop once remaining < ASSET_STORE_PROBE_FLOOR_MS.
    """
    # explicit s3:// handled inside s3 fetch
    if explicit and explicit.startswith("s3://"):
        hit = _s3_try_fetch_product_asset(product_id, asset_root, explicit, deadline_ms)
        if hit:
            return hit
        # fall through to local explicit check

    # S3 first (cached) — keeps the asset store usable when input_assets empty but bucket populated
    s3_hit = _s3_try_fetch_product_asset(product_id, asset_root, deadline_ms=deadline_ms)
    if s3_hit:
        return s3_hit

    # local fallback
    return _local_find_hero(product_id, asset_root, explicit)


def find_brand_logo(asset_root: Path) -> Path | None:
    """Return brand logo path if found, else None. S3-first, local fallback.

    Checks s3://$ASSET_STORE_S3_BUCKET/$ASSET_STORE_S3_PREFIX{brand/,}logo.* then local asset_root/brand/logo.* .
    """
    s3_hit = _s3_try_fetch_brand_logo(asset_root)
    if s3_hit:
        return s3_hit
    return _local_find_brand(asset_root)


def s3_upload_and_presign(local_path: Path, key: str, expires: int = 3600) -> str | None:
    """Upload local_path to the asset store bucket under <prefix><key>, return a presigned GET url.

    Mirrors the module's graceful pattern: returns None when S3 is disabled (no
    ASSET_STORE_S3_BUCKET / boto3 missing / client init fails) so callers fall back to a
    local path in offline / CI mode. The key is joined to the configured asset prefix
    ...
    if not local_path.exists():
        print(f"[asset-store] presign skipped — local artifact absent: {local_path}")
        return None
    bucket, prefix = _s3_bucket_and_prefix()
    client = _s3_client()
    ...
    try:
        ...
        print(f"[asset-store] uploaded {local_path} -> s3://{bucket}/{full_key} (presigned {expires}s)")
        return url
    except (ClientError, BotoCoreError, Exception) as e:  # noqa: BLE001 — upload failure returns None
        print(f"[asset-store] upload/presign failed s3://{bucket}/{full_key}: {e}")
        return None


def presign_get(key: str, expires: int = 3600) -> str | None:
    """Presign a GET for an EXISTING asset key. Returns url, or None when S3 disabled.

    Read-only companion to s3_upload_and_presign (which uploads then presigns). The
    key is used VERBATIM against the asset store bucket with NO prefix join — the asset
    browser passes full keys (brands/kodiak/...) that are not under ASSET_STORE_S3_PREFIX,
    same contract as fetch_asset_key. Offline-safe: returns None when S3 is disabled
    (no ASSET_STORE_S3_BUCKET / boto3 missing / client init fails) so callers degrade to a
    ...
    try:
        ...
        print(f"[asset-store] presign_get failed s3://{bucket}/{key}: {e}")
        return None


def head_metadata(key: str) -> dict:
    """HEAD one asset key and return its user metadata (x-amz-meta-*) lowercased.
    ...
    try:
        ...
        print(f"[asset-store] head_metadata failed s3://{bucket}/{key}: {e}")
        return {}
...
# live in account 946179428633 (bryanchasko-kiro), same asset store bucket as the rest of the
# kodiak brand. Overridable via ASSET_STORE_RECIPE_ART_PREFIX for other brands / test buckets.
def _recipe_art_prefix() -> str:
    prefix = _env_compat("ASSET_STORE_RECIPE_ART_PREFIX", "DAM_RECIPE_ART_PREFIX", "brands/kodiak/recipe-art/")
    if prefix and not prefix.endswith("/"):
        prefix += "/"
    return prefix
...
def recipe_art_key(subject_slug: str, zone: str) -> str:
    """Full asset key for a recipe-art asset: <prefix><subject_slug>/<zone>.png."""
    return f"{_recipe_art_prefix()}{subject_slug}/{zone}.png"


def recipe_art_exists(subject_slug: str, zone: str) -> bool:
    """HEAD the recipe-art key — True when the object already exists in the asset store.
...
def recipe_art_site_url(subject_slug: str, zone: str) -> str:
    ...
    relative url would resolve wrong there). The deploy script mirrors the asset store
...
def upload_recipe_art(local_png: Path, subject_slug: str, zone: str) -> str | None:
    ...
    Mirrors publish_card: the full key is relativized against the configured asset store
    ...
    if not local_png.exists():
        print(f"[asset-store] recipe-art upload skipped — local png absent: {local_png}")
        return None
...
def sync_assets_from_s3(asset_root: Path, delete: bool = False) -> bool:
    ...
    """
    if not _s3_enabled():
        print("[asset-store] sync skipped — ASSET_STORE_S3_BUCKET not set or boto3 missing")
        return False
    bucket, prefix = _s3_bucket_and_prefix()
    client = _s3_client()
    if not bucket or client is None:
    ...
    try:
        ...
        for page in paginator.paginate(Bucket=bucket, Prefix=prefix):
            for obj in page.get("Contents", []):
                ...
                if _s3_download(bucket, key, dest):
                    synced += 1
        print(f"[asset-store] sync s3://{bucket}/{prefix} -> {asset_root} synced={synced}")
        return synced > 0
    except (ClientError, BotoCoreError, Exception) as e:  # noqa: BLE001 — sync failure returns False
        print(f"[asset-store] bulk sync failed: {e}")
        return False
