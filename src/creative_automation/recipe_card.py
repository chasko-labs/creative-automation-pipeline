...
def _pick_recipe_detail(
    ...
    if ingredient:
        # --- resilient exact-match: normalize qualifiers before comparing featured_for ---
        # Monthly ingredients carry parenthetical qualifiers that are display truth
        # but must not fork the recipe match. Examples from retailer-frontier-pairs.json:
        #   "tomatoes (late harvest)" -> "tomatoes"
        #   "Vidalia onions (early)"  -> "vidalia onions"
        #   "strawberries (late)"     -> "strawberries"
        #   SoCal: "pumpkins (Julian)", "pears (late)", "heirloom tomatoes (late harvest)"
        #   Georgia: "Vidalia onions (early)" (Atlanta/Senoia); Ohio: "tomatoes (late harvest)"
        #   (Dayton), "strawberries (late)" (Cincinnati/Dayton share Lebanon orchard belt)
        # Bug was exact-match/substring against the raw "tomatoes (late harvest)" was
        # brittle: qualifiers prevented resilient matching. Fix: strip parenthetical
        # qualifiers and lowercase on BOTH sides before comparing featured_for, then
        # use resilient containment (featured_for normalized must be substring of
        # normalized ingredient). Examples: "tomatoes (late harvest)" -> "tomatoes"
        # matches featured_for "tomatoes"; "Half Moon Bay pumpkin" -> still matches
        # "pumpkin"; "Vidalia onions (early)" -> "vidalia onions". Handles Ohio
