...
| Products | `data/products/kodiak-full-catalog.json` (88 SKUs) | `slug`, `product_image` asset key | `catalog` → `generate.js` product picker, `DEFAULT_MAPPED_SLUG` only when explicit `product_image` layer checked. |
