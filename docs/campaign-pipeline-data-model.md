...
|-------|------|---------------|--------------------------|
| Frontier pairs | `data/localization/retailer-frontier-pairs.json` (76 pairs, schema `retailer-frontier-pairs.schema.json`) | `market` → `frontier_sister` (place, farmers_market_url), `monthly_ingredients` (12 months `2026-01`..`2026-12` → 912 cells), `seasonal_moments[]` (264 distinct moments, each with `months[]`, `available_ingredients`, `favorite_flavors`, `note`) | Frontend does not `if (market === "Cincinnati")` — it does `KODIAK_FRONTIER_PAIRS.find(p => p.market === selectedMarket)` for **any** of the 76 the UI lists. No hardwired market branches. |
