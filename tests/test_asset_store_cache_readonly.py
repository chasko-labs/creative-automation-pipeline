"""asset store fetch cache must write to a writable /tmp root, never read-only input_assets.
...
returned 503. See asset_store.py::_asset_store_cache_root / _s3_download.
...
# --------------------------------------------------------------- cache-root resolver
def test_asset_store_cache_root_defaults_to_tmp(monkeypatch) -> None:
    """No override -> /tmp/kodiak-assets, never the relative read-only input_assets."""
    monkeypatch.delenv("ASSET_STORE_CACHE_ROOT", raising=False)
    root = asset_store._asset_store_cache_root()
    assert str(root).startswith("/tmp/"), f"cache root not under /tmp: {root}"
    assert root != Path("input_assets")


def test_asset_store_cache_root_honors_env_override(monkeypatch, tmp_path: Path) -> None:
    monkeypatch.setenv("ASSET_STORE_CACHE_ROOT", str(tmp_path / "custom-cache"))
    assert asset_store._asset_store_cache_root() == tmp_path / "custom-cache"


def test_legacy_env_names_still_resolve(monkeypatch, tmp_path: Path) -> None:
    """Already-deployed environments export the legacy names — they keep working."""
    monkeypatch.delenv("ASSET_STORE_CACHE_ROOT", raising=False)
    monkeypatch.setenv("DAM_CACHE_ROOT", str(tmp_path / "legacy-cache"))
    assert asset_store._asset_store_cache_root() == tmp_path / "legacy-cache"


def test_new_env_name_wins_over_legacy(monkeypatch, tmp_path: Path) -> None:
    monkeypatch.setenv("ASSET_STORE_CACHE_ROOT", str(tmp_path / "new-cache"))
    monkeypatch.setenv("DAM_CACHE_ROOT", str(tmp_path / "legacy-cache"))
    assert asset_store._asset_store_cache_root() == tmp_path / "new-cache"


# -------------------------------------------- fetch writes to /tmp cache, not asset_root
def test_fetch_product_asset_writes_under_tmp_cache_not_asset_store_root(monkeypatch, tmp_path: Path) -> None:
    """The S3 download lands under _asset_store_cache_root(), not the caller's asset_root."""
    cache = tmp_path / "tmp-cache"
    monkeypatch.setattr(asset_store, "_asset_store_cache_root", lambda: cache)
    ...
    # the write landed under the /tmp cache root, NOT under asset_root
    assert str(hit).startswith(str(cache)), f"fetch wrote outside cache root: {hit}"
    assert asset_root not in hit.parents, "fetch wrote into the read-only asset_root"
...
def test_s3_download_mkdir_guard_degrades_gracefully(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr(asset_store, "_s3_enabled", lambda: True)
    monkeypatch.setattr(asset_store, "_s3_client", lambda: object())  # non-None so we reach mkdir
    monkeypatch.setenv("ASSET_STORE_S3_BUCKET", "test-asset_store-bucket")
    try:
        # dest under the read-only dir -> mkdir raises OSError internally -> False, no raise
        ok = asset_store._s3_download("test-asset_store-bucket", "asset_store/x/hero.png", readonly / "x" / "hero.png")
        assert ok is False
    finally:
...
def test_local_read_from_input_assets_still_resolves(monkeypatch, tmp_path: Path) -> None:
    """find_hero_asset glob still reads a committed local asset under asset_root."""
    monkeypatch.setattr(asset_store, "_s3_enabled", lambda: False)  # S3 off -> pure local path
    asset_root = tmp_path / "input_assets"
    local_hero = asset_root / "power-cakes" / "hero.png"
    _write_png(local_hero)
    hit = asset_store.find_hero_asset("power-cakes", asset_root)
...
def test_presynced_local_hit_preferred_over_download(monkeypatch, tmp_path: Path) -> None:
    """A pre-synced asset under asset_root is returned; its key is never downloaded."""
    cache = tmp_path / "tmp-cache"
    monkeypatch.setattr(asset_store, "_asset_store_cache_root", lambda: cache)
    monkeypatch.setattr(asset_store, "_s3_enabled", lambda: True)
    monkeypatch.setenv("ASSET_STORE_S3_BUCKET", "test-asset_store-bucket")
    asset_root = tmp_path / "input_assets"
    presynced = asset_root / "power-cakes" / "hero.png"
    _write_png(presynced)

    downloads: list[str] = []
    monkeypatch.setattr(asset_store, "_s3_download", lambda b, k, d: downloads.append(k) or False)

    hit = asset_store._s3_try_fetch_product_asset("power-cakes", asset_root)
    # the pre-synced local copy is returned (read from asset_root, no cache write for it)
