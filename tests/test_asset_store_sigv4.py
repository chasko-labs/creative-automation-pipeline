"""SigV4 gate (#189): asset store S3 client must force signature_version=s3v4.

KMS-SSE buckets reject SigV2 presigns with 400 InvalidArgument; asset_store.presign_get
...
def test_s3_client_forces_sigv4(monkeypatch) -> None:
    ...
    assert captured.get("signature_version") == "s3v4", (
        f"asset_store._s3_client Config must set signature_version='s3v4', got {captured!r}"
    )
...
def test_presign_get_uses_sigv4_client(monkeypatch) -> None:
    ...
    src = inspect.getsource(asset_store.presign_get)
