"""DAM fetch must use bounded get_object + body.read(), never download_file.
...
def test_s3_download_uses_get_object_writes_dest(monkeypatch, tmp_path: Path) -> None:
    ...
    ok = dam._s3_download("test-dam-bucket", "brands/kodiak/heroes/power-cakes/hero.png", dest)
...
def test_dam_get_object_is_the_only_transfer_api(monkeypatch, tmp_path: Path) -> None:
    ...
    hit = dam.fetch_hero_to_tmp("power-cakes", cache_root=tmp_path / "cache")
