...
# a key prettier leaves unquoted under quoteProps "as-needed": a bare JS identifier.
# numeric-looking ("0", "50") and dotted ("headline.1x1") keys stay quoted, matching
...
def _ts_key(key: str) -> str:
    """Quote a key only when prettier would (quoteProps: as-needed)."""
    return key if _JS_IDENT_RE.match(key) else json.dumps(key, ensure_ascii=False)


def _ts_scalar(val) -> str:
    """Render a leaf value the way prettier emits it: double-quoted strings,
    bare numbers/bools. json.dumps handles escaping and picks double quotes."""
    return json.dumps(val, ensure_ascii=False)


def _ts_object(node, indent: int) -> str:
...
def render_config(theme: dict) -> str:
    ...
    # render the theme extend body, re-indented under `extend:` (indent level 3).
    theme_block = "\n".join(
        f"      {key}: {_ts_object(value, 3)}," for key, value in theme.items()
    )
